...
class Viewer:
    ...
    def draw_surface(self, agents, actions=None):
        ...
        for agent in agents:
            ...
            if agent.type == 'ship':
                pygame.draw.ellipse(self.surface, ship_color, (agent.y * interval, agent.x * interval, interval, interval))
        ...
        if actions is not None:
            ...
            for agent, action in zip(agents, actions):
                ...
                if action > 35:
                    ...
                    memo = f.render(f"Agent_{str(agent.idx)}'s HP is {agent.hp}, moving towards {direction[action%36]}", True, BLACK)
                else:
                    fired_y = action % 9
                    if agent.idx < 3:
                        fired_x = action // 9 + 5
                    ...
                    pygame.draw.rect(self.surface, (150, 5, 5), (fired_x*50, fired_y*50, 50, 50))
